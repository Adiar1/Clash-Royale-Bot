...
from utils.helpers import sanitize_tag, LEVEL_EMOJIS, EMOJI_TROPHYROAD, EVOLUTION_EMOJI, LEVEL_15_EMOJI, LEVEL_14_EMOJI, \
    LEVEL_13_EMOJI, CW2_EMOJI, CC_EMOJI, GC_EMOJI, FAME_EMOJI, MULTIDECK_EMOJI, rankedMEDAL_EMOJI, LEAGUE_IMAGES, \
...
async def handle_player_command(interaction: Interaction, user_or_tag: str):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            # Get player info first to check for seasonal trophies
            ...
            # Get trophies (with seasonal trophy logic)
            ...
            # Reconstruct results list with player_info, trophies, and best_trophies at the start
            ...
            await handle_function(interaction, full_results)
...
async def handle_player_data(interaction, results, above_5000):
    ...
    ranked_info = results[6] if above_5000 else None
    ...
    embed = create_player_embed(player_info, player_tag, trophies, best_trophies, cards, badges, clan_tag,
                                ...
                                ranked_info, above_5000)

    await interaction.followup.send(embed=embed)


def create_player_embed(player_info, player_tag, trophies, best_trophies, cards, badges, clan_tag,
                        current_fame, current_decks_used, last_fame, last_decks_used, ranked_info, above_5000):
    embed = discord.Embed(
    ...
    # Ranked Info (for players above 5000 trophies)
    if above_5000 and ranked_info:
        ranked_current_display = format_ranked_entry(ranked_info['current'])
        ranked_best_display = format_ranked_entry(ranked_info['best'])
        embed.add_field(name="**__Ranked__**",
                        value=f"Current: {ranked_current_display}\nBest: {ranked_best_display}", inline=False)
        current_league = ranked_info['current'].get('leagueNumber')
...
def format_ranked_entry(ranked_entry):
